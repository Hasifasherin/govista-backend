...
// Helper: Normalize date to UTC midnight to prevent timezone issues
...
export const getOperatorBookings = async (
  ...
  try {
    ...
    const bookings = await Booking.find({ tourId: { $in: tourIds } })
      ...
      .populate("userId", "firstName lastName email") // safer than 'name'
      .sort({ createdAt: -1 });
...
export const cancelBooking = async (
  ...
  try {
    ...
    await createNotification({
      ...
      message: `${userName} has cancelled their booking for "${tour.title}".`,
      type: "booking"
    });
